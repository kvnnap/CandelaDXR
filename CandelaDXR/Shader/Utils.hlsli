...
// inc = 2891336453u when pcg is used for one stream
PRNGState rand_init_from_state(uint state, uint initseq = 1445668226u)
{
    PRNGState r;
    r.state = state;
    r.inc = (initseq << 1u) | 1u; // This is constant
    return r;
}

PRNGState rand_init(uint initstate, uint initseq = 1445668226u)
{
    PRNGState r;
...
uint rand_next_u32(inout PRNGState r)
{
    uint oldstate = r.state;
    r.state = r.state * 747796405u + r.inc;
    uint word = ((oldstate >> ((oldstate >> 28u) + 4u)) ^ oldstate) * 277803737u;
    uint ret = (word >> 22u) ^ word;
    return ret;
}
...
uint chooseInRange(inout PRNGState r, uint a, uint b)
{
    return a + chooseInRange_other(r, b - a + 1);
}

float3 samplePointOnTriangle(inout PRNGState s, float3 verts[3], out float2 uv)
{
...
float3 randomRayLobe(inout PRNGState s, float3 unitNormal, float n, inout float p)
{
...
float3 randomRayHemisphere(inout PRNGState s, float3 unitNormal)
{
	float p;
...
float3 randomRaySphericalCapBase(inout PRNGState s, inout float p, float oneMinusCosPhiConstant)
{
...
float3 randomRaySphere(inout PRNGState s, inout float p)
{
	// 1.f - cos(pi) = 2.f
    return randomRaySphericalCapBase(s, p, 2.f);
}

float3 randomRaySphericalCap(inout PRNGState s, inout float p, float oneMinusCosPhiConstant, float3 unitNormal)
{
    return transformPointToBasis(unitNormal, randomRaySphericalCapBase(s, p, oneMinusCosPhiConstant));
}

// Type manipulation for light tracer
