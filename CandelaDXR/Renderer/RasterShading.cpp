...
RasterShading::RasterShading(bool computeGBuffer)
	...
	dsvDescriptorSize(),
	pDepthBuffer(),
	computeGBuffer(computeGBuffer), 
	numRenderTargets(computeGBuffer ? 5u : 1u), 
	camera(), 
	winDimensions()
{
...
void RasterShading::draw(wrl::ComPtr<ID3D12GraphicsCommandList> pCurrentCommandList, uint32_t currentBackBufferIndex)
{
	...
	for (const auto &child : rendererResources->scene->getSceneGraph().getFlattenedMeshNodes())
	{
		auto& indexedSpan = rendererResources->scene->getMeshIndexedSpan(child.MeshId);
